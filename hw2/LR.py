...
def linear_regression(X, Y):
    ...
    # use y = X^T w
    # add 1s vertically to X for multiplication
    X = np.vstack((X, np.ones(N)))

    X_XT = np.matmul(X, X.T) # X times its transpose 
    X_Y = np.matmul(X, Y) # X times y
    wb_vector = np.linalg.solve(X_XT, X_Y) # vector of [w, b] by solving (X_XT)x=(X_Y)

    w = wb_vector[:-1].reshape(-1, 1) # reshape into (D, 1) column
    b = wb_vector[-1]
