...
class NaiveBayesClassifier(object):
    ...
    def train(self, training_sentences, training_labels):
        ...
        # Set up BOW based on training set using self.V
        vectorsList = list(self.V.keys())
        shape = (N_sentences, len(self.V))
        bag = np.zeros(shape)
        for i in range(0, N_sentences):
            for word in training_set[i]:
                if word in vectorsList:
                    index = vectorsList.index(word)
                    bag[i][index] = 1
        ...
        # initialize self.prior and self.conditional as arrays of 0s
        self.prior = np.zeros(len(all_classes))
        self.conditional = np.zeros((len(all_classes), len(self.V)))

        # For each class (-1,0,1), determine probabilities
        for c in all_classes: # add label indexes to bag for each class
            c_indexes = []
            for index, label in enumerate(training_labels):
                if label == c:
                    c_indexes.append(index)
            class_bow = bag[c_indexes, :]

            # prior probability p(Y=c), calculated by counting number of sentences in class over total num of sentences
            self.prior[c] = len(c_indexes) / N_sentences

            # conditional probabilities determined by counting the sums of all words in class over total words.
            self.conditional[c, :] = class_bow.sum(axis=0) / class_bow.sum()
    ...
    def predict(self, test_sentence):
        ...
        # BOW 
        test_sentence_bow = np.zeros(len(self.V))
        vectorsList = list(self.V.keys())
        for word in test_sentence: # set BOW rep to 1 if word found
            if word in vectorsList:
                index = vectorsList.index(word)
                test_sentence_bow[index] = 1

        # log(p(c)) + SUM[from 1 to]log[(p(x[d]|c))]
        # tbh don't quite know how I got this working but I think it works
        for c in label_probability.keys():
            label_probability[c] = np.log(self.prior[c])
            logProb = np.log(self.conditional[c, :] * test_sentence_bow + (1 - self.conditional[c, :]) * (1 - test_sentence_bow))
            label_probability[c] += np.sum(logProb)
