...
def Logistic_Regression(X, Y, learningRate=0.01, maxIter=100):
    ...
    ####### TODO: implement logistic regression (You should be able to implement this < 10 lines)
        wx = np.matmul(w.transpose(), X) # (w^T)x
        sig = sigmoid(wx.reshape(N, 1)) # sig(w^Tx)
        minLoss = np.matmul(X, (Y - sig)) / (-N)
        w = w - np.dot(learningRate, minLoss)
...
def Perceptron(X, Y, learningRate=0.01, maxIter=100):
    ...
    ####### TODO: implement perceptron (You should be able to implement this < 10 lines)
            yHat = np.sign(np.matmul(w.transpose(), X[:,n] )) # sign(w^Tx_n)
            if (yHat != Y[n]).any():
                w += np.dot(learningRate, (Y[n] * X[:,n]).reshape(D_plus_1, 1)) #second param is y_n*x_n
